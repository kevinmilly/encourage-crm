...
import { Contact, ContactType } from '@operations/contacts';
...
constructor(private metricService:MetricsService, private store:Store<Contact>) {
  ...
  ngOnInit(): void {
    this.contactSub = this.store.select(selectContacts).subscribe(contacts => {
      ...
      this.topEnvironment = this.metricService.getFriendliestEnvironment(this.contacts.filter(c => c.contactType === 0));
      this.mostImportantFriend = this.metricService.getHighest(this.contacts.filter(c => c.contactType === 0));
      this.mostDistantFriend = this.metricService.getLowest(this.contacts.filter(c => c.contactType === 0));
      this.highestRatedContact = this.metricService.getHighest(this.contacts);
      this.lowestRatedContact = this.metricService.getLowest(this.contacts);
      this.highestRatedFamilyMember = this.metricService.getHighest(this.contacts.filter(c => ![0,14,13].includes(c.contactType)));
      this.mostDistantFamilyMember = this.metricService.getLowest(this.contacts.filter(c => ![0,14,13].includes(c.contactType)));

      this.contactDistribution = this.metricService.getContactDistribution(this.contacts)
